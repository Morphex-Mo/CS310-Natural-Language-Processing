...
if __name__ == "__main__":
    import os
    print(f"[Python] Host: {os.uname().nodename}")
    print(f"[Python] CUDA_VISIBLE_DEVICES: {os.environ.get('CUDA_VISIBLE_DEVICES', '<unset>')}")
    try:
        import subprocess
        print('[Python] nvidia-smi output:')
        print(subprocess.check_output(['nvidia-smi']).decode())
    except Exception as e:
        print(f"[Python] nvidia-smi failed: {e}")
